            ...
            if status := response.json().get("status"):
                if status != 0:
                    raise RequestException(
                        f"使用token获得认证代码失败：{response.json().get('msg')}"
                    )
            return (
                response.json()["data"]["code"]
                if grant_type == 0
                else response.json()["data"]["token"]
            )
    ...
    async def get_scan(cls) -> str:
        async with httpx.AsyncClient() as client:
            ...
            if status := response.json().get("status"):
                if status != 0:
                    raise RequestException(
                        f"获取登录二维码失败：{response.json().get('msg')}"
                    )
    ...
    async def get_scan_status(cls, scan_id: str) -> str:
        async with httpx.AsyncClient() as client:
            ...
            if status := response.json().get("status"):
                if status != 0:
                    raise RequestException(
                        f"获取二维码 scanCode 失败：{response.json().get('msg')}"
                    )
    ...
    async def get_token_by_scan_code(cls, scan_code: str) -> str:
        async with httpx.AsyncClient() as client:
            ...
            if status := response.json().get("status"):
                if status != 0:
                    raise RequestException(
                        f"获取token失败：{response.json().get('msg')}"
                    )
    ...
    async def get_role_token_by_uid(cls, uid: str, grant_code: str) -> str:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if status := response.json().get("status"):
                if status != 0:
                    raise RequestException(
                        f"获取role token失败：{response.json().get('msg')}"
                    )
            return response.json()["data"]["token"]
    ...
    async def get_ak_cookie(cls, role_token: str) -> str:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if status := response.json().get("status"):
                if status != 0:
                    raise RequestException(
                        f"获取cookie失败：{response.json().get('msg')}"
                    )
            if not (cookie := response.cookies.get("ak-user-center")):
                raise RequestException(
                    "获取cookie失败：未能获取到 ak-user-center cookie"
                )
            else:
                return cookie
