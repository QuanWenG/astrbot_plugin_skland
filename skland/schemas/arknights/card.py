...
class ArkCard(BaseModel):
    ...
    def inject_uniequip_uris(self) -> Any:
        values = self
        ...
        for char in assist_chars:
            ...
            char.uniequip = (
                f"https://torappu.prts.wiki/assets/uniequip_direction/{equip_id}.png"
            )
        if isinstance(values, dict):
    ...
    @model_validator(mode="after")
    def inject_manufacture_stoke(self) -> Any:
        values = self
        if isinstance(values, dict):
        ...
        for manu in getattr(building, "manufactures", []) or []:
            if manu.formulaId in formula_map:
                ...
                if datetime.now().timestamp() >= manu.completeWorkTime:
                    ...
                    to_be_processed = (manu.completeWorkTime - manu.lastUpdateTime) / (
                        cost_time / manu.speed
                    )
                    has_processed = to_be_processed - int(to_be_processed)
                    additional_complete = (
                        elapsed_time - has_processed * cost_time
                    ) / cost_time
