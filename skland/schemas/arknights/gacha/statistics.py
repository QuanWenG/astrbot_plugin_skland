...
class GroupedGachaRecord(BaseModel):
    ...
    GACHA_RULE_TYPES: ClassVar[dict[str, list[int]]] = {
        "limit": [1, 2, 3, 8],
        "norm": [0, 5, 9],
        "doub": [4, 6, 7, 10],
    }
    ...
    def sort_pools(cls, values) -> Any:
        if "pools" in values:
            values["pools"] = sorted(
                values["pools"], key=lambda x: x.openTime, reverse=True
            )
        return values

    @model_validator(mode="after")
    def calculate_gacha_title(self) -> Any:
        """自动计算并填充抽卡称号"""
        values = self
    ...
    def _sum_by(self, attr: str, group: str) -> int:
        ...
        return sum(
            getattr(pool, attr) for pool in self.pools if pool.gachaRuleType in ids
        )
    ...
    def limit_six_avg(self) -> float:
        ...
        return (
            round(self._sum_by("bare_six_consume", "limit") / total, 1)
            if total
            else 0.0
        )
    ...
    def norm_six_avg(self) -> float:
        ...
        return (
            round(self._sum_by("bare_six_consume", "norm") / total, 1) if total else 0.0
        )
    ...
    def doub_six_avg(self) -> float:
        ...
        return (
            round(self._sum_by("bare_six_consume", "doub") / total, 1) if total else 0.0
        )
    ...
    def up_rate(self) -> float:
        ...
        return round(
            (self.non_doub_six - self.total_spook) / self.non_doub_six * 100, 1
        )
    ...
    def calculate_title(self) -> GachaTitleInfo:
        ...
        total_score = round(
            self._calculate_score(six_star_rate, RATE_SCORES) * 0.7
            + self._calculate_score(up_rate, UP_SCORES) * 0.3,
            2,
        )

        title = next(name for threshold, name in TITLES if total_score >= threshold)

        return GachaTitleInfo(
            title=title,
            six_star_rate=six_star_rate,
            up_rate=up_rate,
