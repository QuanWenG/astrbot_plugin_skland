...
from .base import EfGachaGroup, EfGachaPull
...
class EfGachaPoolInfo(BaseModel):
    ...
    def sort_records(cls, values) -> Any:
        if "records" in values:
            values["records"] = sorted(
                values["records"], key=lambda x: x.gacha_ts, reverse=True
            )
        return values
    ...
    def paid_pulls(self) -> int:
        """该卡池的付费抽数（不含 isFree）"""
        return sum(
            1 for record in self.records for pull in record.pulls if not pull.is_free
        )
    ...
    def free_pulls(self) -> int:
        ...
        return sum(
            1 for record in self.records for pull in record.pulls if pull.is_free
        )
    ...
    def total_six_stars(self) -> int:
        """该卡池的总六星数"""
        return sum(
            1 for record in self.records for pull in record.pulls if pull.rarity == 6
        )
    ...
    def arsenal_quota_earned(self) -> int:
        ...
        """
        return sum(
            ARSENAL_QUOTA_EARN_MAP.get(pull.rarity, 0)
            for record in self.records
            for pull in record.pulls
        )
    ...
    def has_pulled_up_six(self) -> bool:
        ...
        if not self.up_six_chars:
            return False
        return any(
            pull.rarity == 6 and pull.item_id in self.up_six_chars
            for record in self.records
            for pull in record.pulls
        )
