...
async def render_rogue_info(
    props: RogueData, bg: str | Url, id: int, is_favored: bool
) -> bytes:
    return await template_to_pic(
        template_path=str(TEMPLATES_DIR),
        ...
        templates={
            ...
            if is_favored and id - 1 < len(props.history.favourRecords)
                ...
                props.history.records[id - 1]
                if id - 1 < len(props.history.records)
                else None
            ),
...
async def render_ef_card(
    props: EndfieldCard, bg: str | Url, show_all: bool = False, is_simple: bool = False
) -> bytes:
    ...
    if show_all:
        ...
        filtered_chars = [
            char_map[cid] for cid in props.config.charIds if cid in char_map
        ]
    ...
    # 计算理智恢复剩余时间
    current_ts = (
        float(props.currentTs) if props.currentTs else datetime.now().timestamp()
    )
    ...
    stamina_percent = (
        min(100, (cur_stamina / max_stamina) * 100) if max_stamina > 0 else 0
    )

    # Simple 背景模式：命令行参数优先于配置
    simple_bg_enabled = is_simple or config.endfield_background_simple
    simple_bg = (
        RES_DIR / "images" / "background" / "endfield" / "simple" / "simple_bg.png"
    ).as_posix()
    simple_bg_top = (
